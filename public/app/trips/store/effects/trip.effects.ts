...
import { UserFetchUserDetailAction } from 'public/app/users/store/actions/user.action';
    ...
    fetchTripDetail$: Observable<Action> = this.actions$.pipe(
        ...
        switchMap((action: TripFetchTripDetailAction) =>
            this.httpClient.get(`${this.tripsApiUrl}/trip/${action.payload.tripId}`, {
                ...
                switchMap(result => {
                    const trip = this.buildTripCoverUrl(result as Trip);
                    const next = [];

                    trip.participants.forEach(participant => {
                        delete participant.email;
                        delete participant.profilePic;
                        next.push(new UserFetchUserDetailAction({userId: participant.userId}));
                    });

                    next.push(new TripFetchTripDetailSuccessAction({
                        trip: trip
                    }));
                    return next;
